...
def get_current_date() -> int:
    ...
    return int(datetime.datetime.now(cst).strftime("%Y%m%d"))
        ...
        for t in transactions:
            if t.date == current_date and t.amount and t.amount < 0:
                new_transactions.append(
                    Transaction(
                        amount=t.amount / 100 if t.amount else None,
                        payee=t.payee.name if t.payee else None,
                    )
                )

    return TodayTransactions(
        transactions=new_transactions, total=tally_transactions(new_transactions)
    )
